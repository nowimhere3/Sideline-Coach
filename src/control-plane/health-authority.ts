...
// Distinct quota cycles (Claude and Codex) are at least 5 hours apart, so resets this
// close are one cycle. Both providers jitter a cycle's reset by seconds between reads
// (Claude ~1-2s; Codex field rollouts show 3s, 4s and 13s).
...
export function claudeWindowsNotReflected(
  ...
  return CLAUDE_WINDOW_KEYS.filter((key) => {
    ...
    return !sameResetCycle(windowResetsAt(have), windowResetsAt(want));
  });
}

/** Codex counterpart of claudeWindowsNotReflected, over `primary`/`secondary`. */
export function codexWindowsNotReflected(
  snapshot: HealthAuthoritySnapshot,
  acquired: Record<string, unknown>
): CodexWindowKey[] {
  const stored = snapshot.providers.codex?.rateLimitInfo ?? {};
  return CODEX_WINDOW_KEYS.filter((key) => {
    const want = acquired[key];
    if (!isObject(want)) return false;
    const have = stored[key];
    if (!isObject(have) || have.usedPercent !== want.usedPercent) return true;
    return !sameResetCycle(windowResetsAt(have), windowResetsAt(want));
  });
}
...
function mergeClaudeRateLimitInfo(
  ...
  for (const key of CLAUDE_WINDOW_KEYS) {
    ...
    if (incWin !== undefined) {
      ...
      if (sameResetCycle(prevResetsAt, incResetsAt)) {
...
function mergeCodexRateLimitInfo(
  ...
  for (const key of CODEX_WINDOW_KEYS) {
    ...
    if (isObject(incWin)) {
      const prevResetsAt = windowResetsAt(prevWin);
      const incResetsAt = windowResetsAt(incWin);
      // Same cycle despite reset jitter: the fresh read wins (see the Claude merge).
      if (isObject(prevWin) && sameResetCycle(prevResetsAt, incResetsAt)) {
        merged[key] = sameWindowIgnoringReset(prevWin, incWin) ? prevWin : incWin;
        continue;
      }
