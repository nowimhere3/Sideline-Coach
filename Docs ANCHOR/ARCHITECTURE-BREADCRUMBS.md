...
- [WHY: Stage 2 / UI-1] **Persistent Bottom Scoreboard / Toolbar.** A compact, glanceable persistent awareness surface displays canonical Game, Current Play, Player, Model, Effort, and Execution Status (and eventually capacity, connection, and Stadium info). Principle: *Dispatcher = decision surface. Bottom bar = awareness surface.* Both render the same canonical runtime truth without independent state owners.

- [WHY: Stage 2 / UI-2] **Duplicate Controlled Presentation Label Cleanup.** Controlled presentation displayed duplicate suffix wording (e.g. `CODEX 2 · CONTROLLED · CONTROLLED`). Minor Stage 2 UI cleanup.
