...
test('SB-47. Mobile Expanded instruments: two symmetric provider cards (USAGE | RESET on a fixed shared split), one Local Time | Actions panel, no scroll, en-US short dates; desktop Expanded untouched', async () => {
  ...
  const observedAt = new Date((nowSec - 60) * 1000);
  const withData = () => {
    const h = health({
    ...
    h.providers.claude.observedAt = observedAt.toISOString();
    return h;
  };
  const usShort = (d) => `${d.toLocaleDateString('en-US', { weekday: 'short' })} · ${d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`.toUpperCase();

  const phone = await createPage({ initialHealth: withData(), mobile: true }).start();
  await phone.click(phone.$('aiScoreboardExpandBtn'));
  assert.equal(phone.$('aiScoreboardClaudeCardFiveHourReset').textContent,
    `${usShort(fiveAt)}\n${fiveAt.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}`,
    'e.g. "WED · SEP 23" / "2:58 AM", independent of the phone locale');
  for (const provider of ['Claude', 'Codex']) {
    for (const win of ['FiveHour', 'Weekly']) {
      ...
      assert.match(phone.$(`${id}Reset`).textContent, /^[A-Z]{3} · [A-Z]{3} \d{1,2}\n\d{1,2}:\d{2} [AP]M$/, `${id} reset date + clock kept`);
      assert.match(phone.$(`${id}Countdown`).textContent, /^in /, `${id} countdown kept`);
      assert.equal(phone.$(`${id}Left`).parentNode.classList.contains('is-unknown'), false);
    }
  }
  assert.equal(phone.$('aiScoreboardCodexCardPlan').textContent, 'Plan: plus', 'plan kept');
  assert.equal(phone.$('aiScoreboardClaudeCardObserved').textContent,
    `Observed: ${usShort(observedAt)}, ${observedAt.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}`, 'observed kept, one quiet line');
  assert.match(phone.$('aiScoreboardLocalDate').textContent, /^[A-Z]{3} · [A-Z]{3} \d{1,2}, \d{4}$/, 'local date kept');
  assert.match(phone.$('aiScoreboardLocalClock').textContent, /^\d{1,2}:\d{2}:\d{2} [AP]M$/, 'local clock kept');
  assert.match(phone.$('aiScoreboardLocalTz').textContent, /^Timezone: /, 'timezone kept');

  // A missing window marks its section so "UNKNOWN" is sized to the usage rail.
  phone.emitHealth(health({ claude: { unifiedWindows: { seven_day: { utilization: 0.64, resetsAt: nowSec + 4 * 86400 } } } }));
  await flush();
  assert.equal(phone.$('aiScoreboardClaudeCardFiveHourLeft').textContent, 'UNKNOWN');
  assert.equal(phone.$('aiScoreboardClaudeCardFiveHourLeft').parentNode.classList.contains('is-unknown'), true);
